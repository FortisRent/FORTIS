  ...
  >
    <div class="row q-mb-md">
      <div class="row q-gutter-md q-pa-md">
        <q-btn :to="`/dashboard/tax/${this.$route.params.company_uuid}`" icon="receipt" label="taxas" text-color="black" />
        <q-btn :to="`/dashboard/finance/list/${this.$route.params.company_uuid}`" icon="attach_money" label="Projetos / Financeiro"
        text-color="black" />
        <q-btn :to="`/dashboard/employee/${this.$route.params.company_uuid}`" icon="perm_contact_calendar" label="Colaboradores"
        text-color="black" />
        <q-btn :to="`/dashboard/employee/operator/${this.$route.params.company_uuid}`" icon="front_loader" label="Operadores"
        text-color="black" />
        <q-btn :to="`/dashboard/customers/${this.$route.params.company_uuid}`" icon="group" label="Clientes" text-color="black" />
        <q-btn :to="`/dashboard/role/${this.$route.params.company_uuid}`" icon="diversity_1" label="Cargos" text-color="black" />
        <q-btn :to="`/dashboard/list/machine/${this.$route.params.company_uuid}`" icon="commute" label="Minhas Máquinas" text-color="black" />
        <q-btn :to="`/dashboard/overtime/${this.$route.params.company_uuid}`" icon="browse_gallery" label="HorasExtas" text-color="black" />

        <q-btn :to="`/dashboard/chat`" icon="forum" label="Chat" text-color="black" />
        <q-input
          v-model="searchTerm"
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          field: row => `${row.expected_date || '-'} até ${row.expected_date_end || '-'} às ${row.expected_hour || '-'}`,
          align: 'left',
          sortable: false,
          headerStyle: 'font-size: 18px',
          style: 'font-weight: 400'
    ...
    async get_budget_by_company_uuid() {
      ...
      try {
        ...
        this.rows = data.company_budget.map((item) => ({
          ...
          expected_date_end: item.expected_date_end,
          expected_hour: item.expected_hour,
        }));
