    ...
    <div class="q-pa-md">
      <div class="q-pa-md">
        <p class="text-h6 text-primary text-bold text-center" style="margin: 0">
          Você é responsável pelo pagamento do projeto?
        </p>
        ...
        <q-option-group
          ...
          type="radio"
          color="primary"
          class="q-mt-sm q-ml-xl text-black"
          inline
        />
      ...
      <div class="q-pa-md">
        ...
        <!-- Badge com um dia ou intervalo (usa valores normalizados) -->
...
export default {
  ...
  data() {
    return {
      ...
      // Pode vir string, objeto {from,to} ou array em alguns casos
  ...
  computed: {
    // Normalizador central: transforma qualquer formato em {from, to} válido
    normalized() {
  ...
  methods: {
    ...
    async checkAvailability() {
      try {
        const from = this.normalized.from;
        const to   = this.normalized.to;
        const start = this.hour_start;
        const end   = this.hour_end;

        if (!from || !start || !end) {
          throw new Error('Por favor, selecione a data (ou intervalo) e os dois horários.');
        }

        // Se for um dia só, hora final deve ser maior que a inicial
        ...
        const response = await fetch('https://fortis-api.55technology.com/v1/budget/available/', {
          ...
          body: JSON.stringify({
            ...
            // compat com backend atual: usa hora inicial
            hour_date: start,
            // se o backend já aceitar, você pode enviar também:
            // hour_start: start,
            // hour_end: end,
          }),
        });
    ...
    async confirmBooking() {
      try {
        ...
        if (!from || !to || !start || !end) {
          throw new Error('Por favor, selecione a data (ou intervalo) e os horários antes de confirmar.');
        }
