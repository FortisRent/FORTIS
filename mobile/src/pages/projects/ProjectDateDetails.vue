  ...
  <q-page class="bg-white">
        ...
        <q-item>
          <q-item-section>
            ...
            <q-item-label caption class="text-primary">
              Aqui estão as informações detalhadas do seu agendamento. Por favor, entre em contato
              com a empresa contratada para combinar encontro, pagamento e demais detalhes do serviço.
            </q-item-label>
          </q-item-section>
        </q-item>
      </q-list>
    ...
    <div class="q-pa-md" style="min-width: 300px;">
      <q-list bordered separator style="border-color: secondary;" class="text-primary">
        <q-item>
          <q-item-section>
            <q-item-label overline class="text-primary q-pa-sm"><strong>Mande mensagem e tire sua dúvida</strong></q-item-label>
            <q-btn label="CHAT" color="orange"></q-btn>
          </q-item-section>
        </q-item>
      </q-list>
    </div>
    ...
    <div v-else class="q-pa-md" style="min-width: 300px">
      <q-list bordered separator style="border-color: secondary; border-radius: 5px" class="text-primary">
        ...
        <q-item>
          <q-item-section style="display:flex; gap:.5rem">
            <q-item-label class="text-weight-light">
              <strong class="text-bold">Nome do cliente:</strong> {{ user_name }}
            </q-item-label>
          </q-item-section>
        </q-item>
    ...
    <div v-else class="q-pa-md" style="min-width: 300px">
      <q-list bordered separator style="border-color: secondary; border-radius: 5px" class="text-primary">
        <q-expansion-item label="Informações Detalhadas do Solicitante" header-class="text-primary text-h6">
    ...
    <!-- Máquinas / Orçamento (visualização) -->
    <div v-if="loading" class="q-pa-md" style="min-width: 300px">
      <q-list bordered separator style="border-color: secondary; border-radius: 5px" class="text-primary">
        <q-item><q-item-section><q-item-label><q-skeleton type="text" /></q-item-label></q-item-section></q-item>
      </q-list>
          ...
          <q-form class="q-pa-md">
            ...
            <div v-if="machine.price_per_hour != '0,00'">
              ...
              <p class="text-primary text-bold">
                Franquia mínima de {{ machine.budget_machine_minimum_rental_period || 10 }} horas
              </p>
              ...
              <!-- Distância total (se houver no orçamento) -->
              <q-input
      ...
      <q-list bordered separator style="border-color: secondary; border-radius: 5px" class="text-primary">
        <q-expansion-item label="Licenças, Taxas e Outros" header-class="text-primary text-h6">
          <div v-for="(item, index) in documentItems" :key="index" class="q-pa-md">
            ...
            <div class="row q-col-gutter-md">
              <div class="col-12 col-md-4">
                <q-input :model-value="item.fee_amount" readonly filled label="Valor taxas" :prefix="'R$ '" />
              </div>
          ...
          <div class="q-pa-md">
            ...
            <p class="q-mb-md">{{ budget_observation }}</p>
          </div>
      ...
      <q-list bordered separator style="border-color: secondary;" class="text-primary">
        <q-item>
          <q-item-section>
            ...
            <q-item-label caption class="text-primary">
              O cancelamento pode ser feito no máximo de 24 horas antes do serviço.
            </q-item-label>
            <q-btn label="DESEJA CANCELAR?" color="red" class="q-ma-sm" @click="confirmSubmission" />
          </q-item-section>
        </q-item>
      </q-list>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      // distância total (se enviada no orçamento)
      total_distance: 0,
    };
  },
  ...
  methods: {
    ...
    normalizeToCents(v) {
      ...
      if (typeof v === 'string') return Math.round(this.parseMoney(v) * 100); // "1.234,56" -> 123456
      if (!Number.isInteger(v))  return Math.round(Number(v) * 100);         // 1234.56 -> 123456
      return v; // já em centavos
    },
    parseDistance(v) {
    ...
    async get_budget_by_project_uuid() {
      await fetch(`https://fortis-api.55technology.com/v1/budget/details/${this.$route.params.budget_uuid}`, {
        headers: { token: localStorage.getItem('access_token') },
      })
        .then((r) => {
          if (!r.ok) throw new Error('Network response was not ok');
        ...
        .then((data) => {
          ...
          // distância total (se backend enviar)
          ...
          // totais (strings pt-BR -> centavos)
          const rawTotalFee = (data.total_fee ?? data.budget?.total_fee ?? null);
          const rawAmount   = (data.amount    ?? data.budget?.amount    ?? null);
          this.total_fee_cents = this.normalizeToCents(rawTotalFee) ?? 0;
          this.amount_cents    = this.normalizeToCents(rawAmount)    ?? null;

          // se não veio amount, recalcula: taxas + franquia + distância
