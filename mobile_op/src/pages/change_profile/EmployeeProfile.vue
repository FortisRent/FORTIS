	...
	<q-page class="bg-white">
		...
		<div class="q-pa-md">
			<q-card class="card-style q-pa-sm q-mb-md">
				<div class="text-center text-caption">
					<q-avatar size="90px" class="q-mb-lg q-mt-md">
						<q-img v-if="profile_picture_url" :src="`http://localhost:5510/${profile_picture_url}`" width="90px"
			...
			<q-card class="card-style q-pa-sm q-mb-md">
				...
				<div v-if="loaded" class="text-left q-pa-sm text-caption">
					<p class="text-primary no-margin text-truncate">Nome: {{ company_name }} </p>
					<!-- <p class="text-primary no-margin text-truncate">Telefone: {{ birthdate }} </p> -->
					<p class="text-primary no-margin text-truncate">Cargo: {{ role_name }} </p>
				</div>
				<q-skeleton v-else type="text" width="200px" />
			</q-card>
	...
	methods: {
		...
		async get_employee_by_logged() {
			try {
				...
				this.company_name = data.employees[0].company_name;
			} catch (error) {
				this.$q.notify({
					color: "red-5",
					textColor: "white",
