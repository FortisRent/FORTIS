    ...
    <div class="q-gutter-y-md">
      <q-card>
        <div class="flex justify-center">
          ...
          <p class="text-bold text-h5 text-primary ">Serviço em andamento</p>
        </div>
      </q-card>
            ...
            >
              ...
              >
                ...
                <div class="text-left">
                  ...
                  class="text-green q-mt-auto"
                  > VER LOCALIZAÇÃO
                </a>
              </div>
                   <div v-if="!budget_machine.checkout_date">
                      <router-link
                    ...
                    <div>
                      <p class="text-primary">SERVIÇO INICIADO {{ budget_machine.checkin_date }}
                          {{ budget_machine.checkin_hour }}

                      </p>
                    </div>
                  </div>
                    <div v-if="budget_machine.checkout_date">
                      <p class="text-center text-primary ">SERVIÇO FINALIZADO {{ budget_machine.checkout_date }}<br> {{ budget_machine.checkout_hour }}</p>
                    </div>
              </div>
              <!--  -->
            </q-btn>
...
export default {
  ...
  computed: {
    ...
    //     .filter((project) => project.status_name === 'ACEITO')
    //     .filter((project) =>
  ...
  methods: {
    ...
    async get_project_by_logged() {
      this.isLoading = true;
      fetch('https://fortis-api.55technology.com/v1/budget/machine/logged/checkin/', {
        headers: { token: localStorage.getItem('access_token') },
      })
        .then((response) => {
          if (!response.ok) {
        ...
        .then((data) => {
          ...
          this.budget_machines = data.budget_machine.map((budget_machine) => ({
            ...
            checkout_date: budget_machine.checkout_date,
            checkout_hour: budget_machine.checkout_hour,
            checkin_date: budget_machine.checkin_date,
            checkin_hour: budget_machine.checkin_hour,
          }));
        })
        .catch((error) => {
          console.error('Error fetching data:', error);
